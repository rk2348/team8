...
<!-- ▼ キービジュアル（タイトル画面やメインビジュアルの画像に差し替えてください） -->
...
29. [技術的な工夫（Technical Highlights）](#29-技術的な工夫technical-highlights)
...
36. [未確定事項・検討課題（Open Questions）](#36-未確定事項検討課題open-questions)
37. [今後の展望・拡張構想（Future Works）](#37-今後の展望拡張構想future-works)
...
### チームの得意な技術・経験・興味の掛け合わせ

- 黒川：Unity/VR開発（Meta Quest向けタイトル開発の経験）× 憑依システムというコア機能への集中実装
- 小林・相山：Blenderでの3Dモデリング・アニメーション制作経験
- 谷田・有谷：空間オーディオ実装・動物の生態情報リサーチの知見

これらの技術・経験を掛け合わせることで、限られた開発期間でも成立する体験設計とした。

### 限られた時間で形にし、デモで魅力を伝えられる案にする工夫

- 24〜30時間という制約の中、機能を「トラへの憑依１本」に絞り込み、**探索→発見→憑依→狩り→リザルト**という一本のプレイループをデモでそのまま完結できる設計にした。
- 事前準備期間はHackit運営の規定に沿って「環境構築・調査・設計」までに限定し、コア機能の実装はすべて開催期間内で行う（詳細は第19章・第21章）。この制約を逆手に取り、限られた時間の中でコア体験を完成させるプロセス自体も「突破」の実演として発表に組み込む。
...
- **家庭でのプレイ（Home VR / リビングルーム）**
  ユーザーが自宅のプレイスペースでMeta Questを被り、数十分〜1時間程度の没入体験を楽しむ。スタンドアローン動作のため、ケーブルレスで自由に周囲を見渡し、動物の視点に合わせてしゃがむ・見上げるといった身体的なアクションが可能。
...
- **主な動物とその役割:**
  ...
  - **シマウマ:** 群れで行動し、捕食者からの警戒と高速逃走を行う被食者。ボイド的アルゴリズムによる大規模な群れの動きを見せる。
...
## 7. アートディレクション・スタイルガイド

- **基本スタイル:** ローポリゴン＋デフォルメ。「親しみやすい可愛らしさ」と「生態が伝わるリアリティ」のバランスを狙う。
- **カラーパレット:** バイオームごとに象徴色を設定し、世界観を差別化する（例：サファリ＝夕暮れのオレンジ〜サバンナのイエロー）。具体的な配色はムードボードで確定（TBD）。
- **ライティング:** ベイクドライティングを基本とし、時間変化（朝・夕・夜）を演出として活かす。
- **モデリング規約:** ポリゴン数の目安、共通スケール（実寸ベース）、リグ／ボーン名の統一を定め、アニメーションやAIでの流用を容易にする。
- **参考資料:** ムードボード・参考画像は `docs/artref/` に集約する（プレースホルダ）。

<!-- ▼ スタイル基準を示すムードボード／カラーパレット -->
![スタイルガイド／ムードボード](docs/images/style_guide.png)

> ※ 具体的な配色・ポリゴン予算は、モデラー（小林・相山）とTA（谷田）の協議で確定・更新する。

---

## 8. 登場動物データ（Animal Data）

各動物の生息環境・食性・食物連鎖上の役割は、モデリング／AI実装／生存ミッション設計の共通仕様となる。実際のデータは、生態情報リサーチ担当（谷田・有谷）の調査結果に基づき随時更新・確定していく前提の作業用リファレンスである。生態情報の出典は「[31. 参考文献（References）](#31-参考文献references)」に記録する。
...
> ※ 現在の `AnimalIdentity`（第21章）が定義する実装対象種族は、トラ・オオカミ・牛・馬・シマウマ・シカ・ウサギ・ゾウの8種。**ヒツジは生態データとしては用意しているが、現行のスクリプト実装には未反映**のため、実装対象に含めるか、データのみ残して見送るかを要検討（第36章に追加）。
...
- 生存時間、達成した目標数、危険回避などを総合してランク評価（例：S / A / B / C）。
- 結果は「観察記録」としてアーカイブされ、再挑戦のモチベーションとする。

### プレイモード

- **通常モード:** 標準的なサバイバル体験。
- **ショートモード（約5分）:** LBE・イベント向けの短縮版（第5章と連動）。
- **教育モード:** 授業での共有・解説に適した進行（キャスト機能と併用）。

> ※ ゲージの増減量、評価式、制限時間などの具体的パラメータは実機検証で確定する（第36章に集約）。
...
- `HerdBehavior` が、同種の近傍個体の重心（結合）と、近すぎる個体からの反発（分離）を計算し、`AnimalIdleBehavior` の次の歩行先にバイアスとして加算する。群れの中心から一定距離以上離れないための上限（はぐれ防止）も設定できる。
- 群れを作らない種族（トラなど単独行動の動物）は `HerdBehavior` を付けないことで、通常のランダム徘徊のみが行われる。
...
- 群れパラメータ（Herd Radius／Cohesion Weight等）の実機でのバランス調整。

> ※ 各パラメータの具体的な数値は動物ごとにInspectorで調整可能。実機での調整記録は第29章「技術的な工夫」に追記していく。
...
- **実装:** Unity上での再生・空間化の方式（標準機能／ミドルウェアの選定）は担当（谷田・有谷）が検証のうえ決定する（TBD）。
- **素材管理:** 外部サイトから取得した音源は、ダウンロード時点で「[30. クレジット・使用素材（Credits）](#30-クレジット使用素材credits)」の表に必ず記録する。
...
| 移動 | 左サムスティック | 🚧 |
| 視点移動 | 実際の視点移動 | 🚧 |
| 動物に憑依する / 乗り物に乗る・降りる | 右Aボタン | ✅（`AnimalViewSwitch`、距離判定つき） |
| 憑依を解除する | 右Bボタン | ✅（元の位置・向きへ復帰） |
| 鳴き声（咆哮）を発する | 右人差し指トリガー | ✅（`AudioSource.PlayOneShot`） |
| 捕食・その他アクション | 未定 | 📋（現状は右人差し指トリガー＝鳴き声のみ実装） |
| バイオームのマップ確認 | 左メニューボタン | 📋 |
...
- **移動酔い（VR酔い）対策:** 視点回転はスナップターンを標準とし、急激な視界変化を避ける。高速移動時にはビネット（周辺減光／トンネルビジョン）を適用。テレポート移動オプションの併用も検討する。
- **姿勢オプション:** 立位（ルームスケール）と着座の両方に対応。LBEや福祉施設での利用を想定し、着座のまま全方位を見渡せる操作を用意する。
- **プレイ時間への配慮:** LBE向けのショートモード（約5分）や、長時間利用時の休憩を促す仕組みを検討。
- **アクセシビリティ:** 利き手切り替え、UI・字幕の表示、効果音／BGMの個別音量調整、視覚エフェクトの強度調整などを提供予定。
- **共有・観覧支援:** 教育現場向けに、ヘッドセットの視界を外部モニターへミラーリングするキャスト機能を提供し、非装着者も体験を共有できるようにする。

---

## 15. ユーザー体験フロー

1. **起動・エリア選択:** 没入空間のロビーから、探索したいバイオーム（ジオラマ）を選択してダイブ。
2. **VR探索:** ジープや徒歩でバイオーム内を自由に探索し、ターゲットとなる動物を探す。
3. **接触と憑依:** 発見した動物に接近・インタラクトすることで視点を切り替え。
4. **サバイバル体験:** 動物として数分間のサバイバル・シミュレーション（ミッション）を開始。
5. **リザルト:** 生存時間や達成した目標に応じた評価が表示され、観察記録としてアーカイブされる。
...
- **描画方針:** ローポリゴン＋ベイクドライティングを基本とし、リアルタイムライトは最小限に抑える。
- **最適化手法:** GPU Instancing、Occlusion Culling、LOD、テクスチャアトラス化とASTC圧縮、Fixed Foveated Rendering などを活用。
- **予算（Budget）:** 1フレームあたりのドローコール数・ポリゴン数の上限は、実機検証を経て確定する（TBD）。
- **オーディオ:** 空間オーディオ（3D音響）により、視覚負荷を抑えつつ没入感を担保する。
- **モデリング:** Quest単体動作のため、Blenderの Decimate / Remesh で動物1体 5,000〜15,000 tris、小物は500〜2,000 trisを目安に削減する

> ※ 具体的な数値目標は、担当（小林・有谷・谷田）による実機プロファイリングを通じて確定・更新する。
...
Notion上のタスク分けを基に設計した、`Assets/Scripts/` 配下の実装スクリプト一覧である。名称・責務をあらかじめチームで統一しておくことで、当日の並行作業時のファイル競合・命名ブレを防ぐことを目的とする。ハッカソン期間中に実装が完了したものは実装状況を随時更新する（全体構想の約55本のうち、動物AI・憑依・HUD関連の主要スクリプトを先行実装済み）。
...
| `HerdBehavior` | 群れの結合（中心への引き寄せ）・分離（近すぎる仲間からの反発）を計算し、徘徊先にバイアスをかける | 牛・馬・シマウマ・シカ・ゾウ・オオカミ | ✅ |

### `Assets/Scripts/Possession/`（憑依システム）

| スクリプト名 | 責務 | アタッチ対象 | 実装状況 |
| --- | --- | --- | --- |
| `AnimalViewSwitch` | 距離判定に基づく憑依開始（Aボタン）・解除（Bボタン）、視点・位置の同期、鳴き声再生（右人差し指トリガー） | 憑依可能な全動物 | ✅ |
...
| スクリプト名 | 責務 | アタッチ対象 | 実装状況 |
| --- | --- | --- | --- |
| `PossessionController` | 憑依中の4パネル（ミッション／時間帯／場所／体力ゲージ）の表示・非表示・内容更新 | HUD用Canvas | 🚧（表示制御は実装済み。時間帯・体力の実データ連携は未着手） |

> ※ 上記以外の予定スクリプト（乗り物、リザルト画面、生存ゲージのロジック、環境音・BGM制御など）は本書執筆時点で未着手（📋）。実装が進み次第この表を更新する。
...
Hackit運営が定める事前準備ルールでは、開催期間前の作業は「環境構築・調査・設計」の範囲に限定されており、コアとなるゲームプレイのコード実装は一切行っていない（第3章・第19章・第21章・第30章を参照）。本章では、その制約の中でチームが事前に完了させた準備内容を、審査・引き継ぎ時の確認用に一覧化する。
...
| WBS作成 | 1,000件超のタスクからなるExcelベースのWBSを作成し、事前準備／開催期間中のタスクを仕分け | 第24章・第25章 |
...
| 画面モックアップの作成 | タイトル・探索・憑依・サバイバル・リザルトなど主要画面、計8画面のモックアップを作成 | 第15章 |
| プレゼン資料の構成設計 | 発表用スライドを9枚構成（概要・課題・機能・デモ・技術・今後の展望等）で設計 | 第3章 |
...
### 事前準備における制約（Hackit運営ルールの遵守）

- コアとなるゲームプレイ（憑依システム、動物AI、サバイバル・ミッション等）のC#コーディングは、すべてHackit2026開催期間内（8月1日〜3日、第25章）に行う。
- 事前に用意したのは「何を・どう作るか」を定める設計情報（WBS・スクリプト構成・スタイル方針など）であり、実行可能なプログラムそのものは含まない。
- 生成AI（Claude等）の活用も、事前準備段階では企画書・WBS・スクリプト設計などドキュメント作成支援にとどめ、コード生成そのものは開催期間中のみ行っている（第30章）。

> ※ 上記は事前準備段階（〜7月31日）で完了した内容のリファレンス。開催期間中の実装進捗は第27章「実装状況」、発表直前の最終状態は第29章「技術的な工夫」と合わせて確認すること。
...
| 時間 | チーム全体の目標・マイルストーン | 各担当の主なタスク |
| --- | --- | --- |
| 09:50 - 12:00 | **環境構築とベースシーン作成** | **Unity:** LFS/GitHub連携確認、ベース地形作成、移動処理<br>**Blender:** トラ・シマウマの基礎モデリング開始<br>**Audio/TA:** 環境音・足音の選定、マテリアル設定準備 |
| 13:00 - 16:00 | **憑依システムのプロトタイプ実装** | **Unity:** ターゲットへの接近判定、視点切り替え処理<br>**Blender:** メイン動物のボーン・待機/走行アニメーション<br>**Audio/TA:** 空間オーディオの実装テスト |
| 16:30 - 19:00 | **狩り・逃走ループの仮組み** | **Unity:** トラの跳躍、シマウマの逃走AI、初期UI（ゲージ類）<br>**Blender:** サバンナの環境プロップ（草・岩・木）の量産 |
...
| 時間 | チーム全体の目標・マイルストーン | 各担当の主なタスク |
| --- | --- | --- |
| 09:10 - 12:00 | **ゲームループの結合と調整** | **Unity:** ミッション進行、リザルト画面、スコア計算<br>**Blender:** ゾウ・シカなど追加動物のモデリング・調整<br>**Audio/TA:** 憑依時のBGM変化、環境音のレイヤー化 |
| 13:00 - 16:00 | **グラフィックとパフォーマンス最適化** | **Unity:** AIの挙動微調整、バグ修正、UIの視認性向上<br>**Blender/TA:** ポリゴン数削減（LOD）、テクスチャ圧縮<br>**全体:** 実機（Quest）でのfps計測と酔い対策（ビネット等） |
| 16:30 - 19:00 | **最終ブラッシュアップ・テストプレイ** | **全体:** 通しプレイでのバランス調整、致命的なバグの洗い出しと修正。発表用スクリーンショット・動画の撮影 |

### 8月3日（月）Day 3：発表準備
**目標:** 安定した最終ビルドの提出と、魅力を伝えるプレゼンテーション。

| 時間 | チーム全体の目標・マイルストーン | 内容 |
| --- | --- | --- |
| 09:10 - 10:00 | **最終ビルド作成（新機能追加禁止）** | 致命的なエラーの修正のみを実施。10:00に向けて実機用APKの最終ビルドとGitHubへのプッシュを完了させる。 |
...
| 相山 莉子 | 3Dモデラー / 動物のアセット作成・アニメーション・環境作成 / 果物作成担当 | Blender |

---

## 27. 実装状況（Implementation Status）

審査時点で「実際に動作するもの」と「構想段階のもの」を明確に区別するための一覧。**ハッカソン期間中、機能が動作した時点で随時更新する。**

**凡例:** ✅ 実装済み（実機動作確認済み） / 🚧 開発中 / 📋 構想・未着手

| 機能 | 状態 | 備考 |
| --- | --- | --- |
| サファリ・エリア環境（地形・ライティング・NavMeshベイク） | 🚧 | NavMesh Surfaceによるベイクは確認済み。ライティング演出は未着手 |
| プレイヤー移動（徒歩・スナップターン） | 🚧 | 既存のVRMovement/CharacterControllerを使用 |
| ジープ搭乗・運転 | 📋 | |
| 動物への憑依（視点切り替え） | ✅ | `AnimalViewSwitch`。距離判定＋Aボタンで憑依、Bボタンで解除、視点への追従HUD込みで実機動作確認済み |
| トラ憑依時のミッション（狩り） | 🚧 | 憑依中の攻撃・捕食アクションは未実装（現状は右人差し指トリガーで鳴き声のみ）。NPCトラ側の狩り・仕留めロジックは実装済み |
| 動物AI（基本ステート：待機・歩く・食べる） | ✅ | `AnimalIdleBehavior`。重み付きランダムでIdle/Walk/Eatを繰り返す |
| 被食者の逃走AI | ✅ | `FleeFromPredators`。捕食者検知・NavMeshでの逃走先計算・Animator連携まで実装済み |
| 捕食者の狩りAI（発見・忍び寄り・追跡・捕食） | ✅ | `PredatorAI`。種族ごとにNotice/Chase/Catch距離を設定可能 |
| 捕食者同士のケンカ・離脱AI | ✅ | `PredatorAI`（Rival Reactions）。一定時間で自動離脱＋クールダウンあり |
| 群れ行動（Boids：結合・分離） | ✅ | `HerdBehavior`。徘徊AIの移動先に群れ補正をかける方式で実装済み |
| 憑依中HUD（ミッション・時間帯・場所・体力） | 🚧 | パネルの表示/非表示・視点追従は実装済み。時間帯・体力の値は現状プレースホルダで、実データ連携は未着手 |
| 鳴き声（咆哮）アクション | ✅ | 右人差し指トリガーでAudioSource.PlayOneShotを再生 |
| 生存ゲージ（体力・空腹） | 🚧 | HUD上の表示枠は実装済み。増減ロジック（空腹・時間経過）は未実装 |
| リザルト画面・ランク評価 | 📋 | |
| 空間オーディオ・環境音 | 🚧 | 動物の鳴き声のみ実装。環境音・BGMは未着手 |
| VR快適性オプション（ビネット等） | 📋 | |
| ショートモード／教育モード | 📋 | 将来構想 |

> ※ 上記は初期状態のテンプレート。**提出直前に必ず実態に合わせて更新すること。** 構想だけで実装が伴わない項目を✅にしないこと（審査でのデモと食い違うと信頼を失う）。

---

## 28. 既知の不具合（Known Issues）

把握済みの不具合と回避策の一覧。審査員のデモプレイ中に発生した場合も「認識済み」であることを示せるよう、発見次第ここに記録する。
...
> ※ 提出時点で未解決の不具合は削除せず、正直に記載する。再現手順が分かるものは Issue にも起票する。

---

## 29. 技術的な工夫（Technical Highlights）

技術審査向けに、本プロジェクトで特に注力した実装・工夫をまとめる。**実装が確定した項目から具体的な数値・手法を追記していく。**

- **憑依システムの視点遷移:** 人間視点⇔動物視点の切り替え時に、カメラ高さ・視野角・移動速度・IPD相当のスケール感を動物ごとのパラメータで補間し、VR酔いを抑えつつ「身体が変わった」感覚を演出する設計。
...
- **Animatorの「Any State」統一設計:** 動物ごとにState数・遷移パターンが異なると管理コストが増大するため、全動物のAnimator ControllerをIdle／Walk／Eat／Run／Attack／Die（捕食者のみAttack）の状態群とし、個別State間の直接遷移ではなく「Any State→各State」への統一遷移で構成。どの状態にいてもTriggerが立った瞬間に確実に反応する設計とし、複数動物への横展開を容易にした。
...
> ※ ハッカソン終了時に「実際にやったこと」ベースで書き直し、可能なら計測値（fps、ドローコール数、tris数など）を添える。
...
| コーディング支援 | Claude / GitHub Copilot | 開催期間中のC#スクリプトの雛形生成、デバッグ補助 |

> 憑依システムのアーキテクチャ設計・実装判断、及び最終的なコードの採否はすべてチームで実施する。

### アセット・音源

- **3Dモデル・アニメーション:** Blenderで自作（生成AI不使用）。
- **音源:** ※使用する場合はツール名と用途をここに記載する（AI生成音源を採用した場合は「[30. クレジット・使用素材（Credits）](#30-クレジット使用素材credits)」にも記録する）。

### 運用ポリシー

- ハッカソン運営のAI利用規約・レギュレーションを遵守し、要求があれば使用範囲を申告する。
- AI生成コードはそのまま採用せず、必ずレビュー・実機検証を経て統合する。
- README・WBS・タスク分けなど「準備」段階の整理にAIを用いた場合と、開催期間中の実装支援にAIを用いた場合を区別して記録し、事前準備／事前開発のルール（第19章・第21章）との整合を保つ。
- AI生成アセットを使用する場合は、ライセンス条件（商用利用可否・クレジット表記）を確認のうえ本節と第31章に明記する。

> ※ 使用ツールに変更・追加があった場合は本節を随時更新する。
...
> **運用ルール:** 素材は**ダウンロードした時点で**必ず以下の表に記録すること（担当：谷田・有谷）。「後でまとめて書く」は出典不明の素材を生む最悪パターンなので厳禁。クレジット表記が必要な素材は、ゲーム内クレジット画面／発表スライドにも記載する。
...
| YouTube | https://www.youtube.com/watch?v=6_ksjDSoIYk | トラの狩猟行動_1 |
| YouTube | https://www.youtube.com/watch?v=0eetPu-jZLY | トラの狩猟行動_2 |
| YouTube | https://www.youtube.com/watch?v=iOvrFgvOuTc | トラの狩猟行動_3 |
| Youtube | https://www.youtube.com/watch?v=ekBuFrBtjqE&t=18s | --- |
| --- | --- | --- |
| --- | --- | --- |
| --- | --- | --- |
| --- | --- | --- |
...
| --- | --- | --- |
| --- | --- | --- |
| Webサイト | 【地上最大の野生動物 ゾウ】https://www.wwf.or.jp/activities/basicinfo/4291.html | ゾウの生態・特徴 |
...
## 36. 未確定事項・検討課題（Open Questions）

各節に散在する未確定（TBD）事項を集約したチェックリスト。決定次第、該当セクションと本表を更新する。

| 項目 | 内容 | 関連セクション |
| --- | --- | --- |
| 対応機種の確定 | Quest 2 の動作可否 | 16 |
| 必要ストレージ | ビルドサイズの確定 | 16 |
| 動作確認済み環境 | 実機・OSビルド番号の記載 | 16 |
| パフォーマンス予算 | ドローコール／ポリゴン数の上限 | 17 |
| カラーパレット | バイオーム別の具体的配色 | 7 |
| 動物データ調査 | 空欄の追加候補の生態データ更新 | 8 |
| ヒツジの扱い | 生態データはあるが `AnimalIdentity` の実装対象（8種）には未反映。実装対象に加えるか、資料上のみに留めるか要決定 | 8, 21 |
| スコア設計 | ゲージ増減・評価式・制限時間 | 10 |
| 動物AI詳細 | ステート遷移・群れロジックは実装済み。各種族のNotice/Chase/Catch距離・群れパラメータは実機でのバランス調整が残課題 | 11 |
| 憑依中のトラのアクション | 現状は鳴き声のみ実装。攻撃・捕食アクションの入力方式を要決定 | 13, 27 |
| オーディオ実装 | 空間オーディオの方式・ミドルウェア選定 | 12 |
| プロジェクト構成 | 実フォルダ規約の確定 | 20 |
| スクリプト一覧の実装状況 | 各スクリプトの実装完了状況の反映 | 21 |
| ブランチ／規約 | チーム合意による確定 | 22 |
| スケジュール | 各マイルストーンの日付 | 25 |
| 実装状況の更新 | 提出直前の実態反映（✅／🚧の確定） | 27 |
| 既知の不具合の記録 | デモ前の最終確認・記録 | 28 |
| 技術ハイライトの具体化 | 実装確定後に計測値を追記 | 29 |
| 音源の生成AI利用 | 使用可否・ツール・ライセンス | 30, 31 |
| 使用素材の記録 | 音源・フォント等のクレジット表の記入 | 31 |
| 生態データの出典 | 参考文献の記録 | 32 |
| 対象年齢の最終確認 | Meta最新規定と教育用途の整合 | 33 |
| ライセンス | 公開・配布方針 | 34 |
| 将来バイオーム | オーシャン等の実装可否・時期 | 6 |
| マルチプレイ | 共有機能の実現性検証 | 24 |

---

## 37. 今後の展望・拡張構想（Future Works）
